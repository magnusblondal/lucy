...
from lucy.application.usecases.bots.audit import (
    ProfitOrLossCalculationForPosition)
...
def position_entry_handler(event: PositionEntryEvent):
    ...
    r.add(event.position_id, event.bot_id, event.symbol, event.side)
...
def profit_calculated_handler(event: ProfitCalculatedEvent):
    ...
    PositionRepository().update_profit(event.position_id,
                                       event.profit, event.profit_percentage)
...
def order_filled_handler(event: OrderFilledEvent):
    logger.info(
        f"Order Filled Handler: {event} Order filled: {event.is_order_filled}")
