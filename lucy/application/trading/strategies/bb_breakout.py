...
class BBbreakout(Strategy):
    ...
    def validate_entry(
        ...
        df["ema_cross_up"] = (df[fastMa] > df[slowMa]) & (
            df[fastMa].shift() < df[slowMa].shift())
        # fast ma is crossing down under slow ma
        df["ema_cross_down"] = (df[fastMa] < df[slowMa]) & (
            df[fastMa].shift() > df[slowMa].shift())
        # closing above fast upper band
        df["bb_breakout"] = np.where(
            df['close'] > df[bbu].shift(), True, False)
        # closing below slow upper band
        df["bb_within_slow"] = np.where(
            df['close'] < df[bbu_slow].shift(), True, False)
        # this is the first close above the fast upper band
        df["bb_signal"] = (
            df['bb_breakout'] is True
        ) & (
            df['bb_breakout'].shift() is False)

        # are going up from basis, rather than down from beyond the upper band
        # coming down from above fast upper band
        df["bbu_slow_cross_down"] = (
            df['close'] < df[bbu_slow]
        ) & (
            df['close'].shift() > df[bbu_slow].shift())

        # close crossing up over middle band
        df["bbm_cross_up"] = (
            df['close'] > df[bbm]
        ) & (
            df['close'].shift() < df[bbm].shift())
        ...
        df['counter_bbu_slow_cross_down'] = df.index.where(
            df.bbu_slow_cross_down)
        ...
        df['delta_bbm_cross_up'] = (df['date'].sub(
            df['counter_bbm_cross_up']
        ).astype(
            'timedelta64[s]').dt.total_seconds()) + 1
        # reikna hvað er langt frá því að fórum niður fyrir slow upper band
        df['delta_bbu_slow_cross_down'] = (df['date'].sub(
            df['counter_bbu_slow_cross_down']
        ).astype('timedelta64[s]').dt.total_seconds()) + 1
        # fórum við síðast upp yfir middle band
        df['bbm_cross_up_last'] = df['delta_bbm_cross_up'] < df['delta_bbu_slow_cross_down'].fillna(
            float('inf'))
        # drop columns
        df.drop(['counter_bbm_cross_up', 'counter_bbu_slow_cross_down',
                'date'], axis=1, inplace=True)

        # Uptrend and BB breakout
        # and BB within slow and coming up from fast basis
        df["entry_signal"] = (
            df["up_trend"] is True
        ) & (df["bb_signal"] is True) & (
            df["bb_within_slow"] is True) & (df["bbm_cross_up_last"] is True)
        ...
        if entry_signal:
            self._chart_entry(df, pair, interval, bbm, bbu,
                              bbu_slow, slowMa, fastMa)

        s = "{} Entry: {} | {} | {} | {} | {} | {} | {} | {}".format(
            pair,
            f"Close: {close:.3f}",
            f"BBM: {df[bbm].iloc[-1]:.3f}",
            f"BBU_fast: {df[bbu].iloc[-1]:.3f}",
            f"BBU_slow: {df[bbu_slow].iloc[-1]:.3f}",
            f"Uptrend: {df['up_trend'].iloc[-1]}",
            f"BBbreakout: {df['bb_breakout'].iloc[-1]}",
            f"Within slow: {df['bb_within_slow'].iloc[-1]}",
            f"Entry: {entry_signal}"
        )
        self.logger.info(s)
    ...
    def validate_add_funds(
        ...
        df['so_signal'] = (df['so_price_below_threshold'] is
                           True) & (df['rsi_cross_up'] is True)
    ...
    def _chart_entry(
        ...
        signals = [trend, entry_signal, bb_breakout, bb_signal,
                   bb_within_slow, up_trend, bbm_cross_up_last]
    ...
    def _chart_tp(
        ...
        tp_rsi_cross_down_trigger = (
            'tp_rsi_cross_down_trigger',
            df[['RSI_50', 'tp_rsi_cross_down_trigger', rsi_col_name]],
        )
