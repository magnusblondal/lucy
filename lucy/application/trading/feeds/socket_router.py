...
class SocketRouter:
    ...
    def __init__(self):
        self.feeds = {
                ...
                'fills_snapshot':       self.fills_snapshot, 
                'fills':                self.fills_update,
                'balances':             self.balances, 
                'balances_snapshot':    self.balances_snapshot,
                'trade_snapshot':       self.trade_snapshot, 
                'trade':                self.trade,
                'notifications_auth':   self.notifications_auth,
                'account_log_snapshot': self.account_log_snapshot, 
                'account_log':          self.account_log
                }
    ...
    def fills_update(self, message_json, event=""):
        ...
        if event == 'subscribed':
            ...
            fs = [ Fill.from_feed(fill) for fill in message_json['fills'] ]
            fills = Fills(fs)
            res = OrderFilled().handle(fills) # TODO: ætti þetta heima í Fills, eða sér usecase?
            print(res)
            # if fills:
            #     for f in fills:
            #         print(f)
            #         res = OrderFilled().handle(f) # TODO: ætti þetta heima í Fills, eða sér usecase?
            #         print(res)                    
            # else:
            #     print('No Fills !!!')
    ...
    def route(self, message_json):
        try:
            ...
            print('>>>>>>> Error ------------------------------')
            print(f"Route Error: {e} {type(e)=} {e.args=}")
            print('-------------------------------------')
