...
class FillsView(View):
    ...
    def lines(
        ...
        for f in fills:
            ...
            tkr_o_type = self.dim('taker order type:', f.taker_order_type)
            order_type = self.dim('order type:', f.order_type)
            p = f"{price} {buy} {qty} {remaining} {fill_type} {fee} {tkr_o_type} {order_type}  {f.time}"
    ...
    def line(
        ...
        fee = self.dim('fee: ', fee)
        tkr_o_type = self.dim('taker order type:', fill.taker_order_type)
        o_type = self.dim('order type:', fill.order_type)
        p = f"{pre} {buy} {qty} {price} {remaining} {fill_type} {fee} {tkr_o_type} {o_type}  {fill.time}"
        print(self.indent(p, indents + 1))
        if verbose:
            print(self.indent(verb, indents + 2))
