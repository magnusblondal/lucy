...
class BotView(View):
    ...
    def listi(self, bots: Bots, header: str="") -> None:
        ...
        for b in bots:
            ...
            table.add_row(b.id.id, self.emp(b.name), is_active, strategy, symbols, interval, max_positions, capital, entry_size, so_size, max_safety_orders, allow_shorts, posCnt, openPos, profit)
        print(table)
    ...
    def show(self, bot: DcaBot, show_signals:bool, verbose: bool=False) -> None:
        ...
        symbols             = self.emp(str(bot.symbols)) if len(bot.symbols) > 0 else self.dim("No symbols")
