...
from lucy.application.events.some_events import (
    PositionEntryEvent,
    # AddFundsEvent,
    PositionExitEvent,
    SignalEvent,
...
class Position(DomainModel):
    ...
    def _create_buy_order(
        ...
        return self.exchange.long_market(
            self.symbol,
            qty,
            self.id.make_combined_id(),
            reduce_only
        ).to_model(self.id, self.bot_id, reduce_only, order_type)
    ...
    def _create_sell_order(
        ...
        return self.exchange.short_market(
            self.symbol,
            qty,
            self.id.make_combined_id(),
            reduce_only
    ...
    # 'serverTime': '2023-07-19T00:27:27.482Z'}
    ...
    def entry(self, signal: Signal, entry_size: float):
        ...
        self._this_just_happened(PositionEntryEvent(
            self.id, self.bot_id, self.symbol, qty, self.side))
    ...
    def add_funds(
        ...
        self._this_just_happened(SignalEvent(signal, self.id))
        if not self.is_open():
            mssg = f"-> Position {self.id} handle {signal.signal_type}"
            print(f"{mssg} -- position is closed, cannot add funds")
            return

        if not self.can_add_safety_order(max_safety_orders):
            p = f"Position {self.id}"
            m = f"Max: {max_safety_orders}"
            o = f"Orders: {len(self.orders)}"
            mssg = " -- Max safety order reached, cannot add funds"
            self.logger.info(f"{p}  {m}  {o} {mssg}")
            return
        ...
        if self.is_open():
            ...
            if self.is_long():
                order = self._create_sell_order(
                    self.open_qty(), 'close', reduce_only=True)
            else:
            ...
            m1 = f"-> Bot {self.id} handle {signal.signal_type}"
            print(f"{m1} -- position is not open, cannot close")
    ...
    def _fetch_executions(self) -> list[Execution]:
        ...
        return self.exchange.executions_by_exchage_ids(
    ...
    # TODO: þetta er óþarfi, má kannski nota til að finna fills, ef það vantar
    def audit(self):
        if self.is_open():
        ...
        if len(self.orders) != len(executions):
            print(
                f"-> Position {self.id} audit -- order count mismatch -> {len(self.orders)} orders, {len(executions)} executions")
            return
        ...
        print(
            f"-> Position {self.id} audit -- profit {self.profit} {self.profit_pct}")
        self._this_just_happened(ProfitCalculatedEvent(
            self.id, self.bot_id, self.profit, self.profit_pct))
...
class Positions(list[Position]):
    ...
    def create_new(self, signal: Signal) -> 'Position':
        symbol = signal.ticker if isinstance(
            signal.ticker, Symbol) else Symbol(signal.ticker)
    ...
    def find_open_by_symbol(self, symbol: Symbol) -> Position:
        return next(
            (p for p in self if p.symbol == symbol and p.is_open()),
            None
        )
