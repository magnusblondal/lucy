...
class FillsRepository(Repository):
    ...
    def add(self, fill: Fill) -> None:
        sql = '''
            INSERT INTO fills (
                id, instrument, time, price, buy, qty, remaining_order_qty,
                order_id, fill_type, fee_paid, fee_currency,
    ...
    def single(self, id: Id) -> Fill:
        sql = '''
            SELECT id, instrument, time, price, buy, qty, remaining_order_qty,
            order_id, fill_type, fee_paid, fee_currency,
            taker_order_type, order_type, cli_ord_id, position_id
            FROM fills
            WHERE id = %s
