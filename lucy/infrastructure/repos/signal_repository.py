...
class SignalRepository(Repository):
    ...
    def add(self, bot_id: Id, position_id: Id, signal: Signal) -> None:
        sql = '''
            INSERT INTO signals (
                id, position_id, bot_id, strategy, ticker, side, signal_type,
                interval, bar_open_time, signal_time, close
            )
            VALUES (
                %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s
            )
            '''
        values = (signal.id.id, position_id.id, bot_id.id, signal.strategy,
                  signal.ticker, signal.side, signal.signal_type,
                  signal.interval.interval, signal.bar_open_time,
                  signal.signal_time, signal.close)
